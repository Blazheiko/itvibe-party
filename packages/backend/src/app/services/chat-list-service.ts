import { contactListRepository, userRepository } from '#app/repositories/index.js';
...
export const chatListService = {
    async getContactList(requestedUserId: number, sessionUserId: string | number | bigint | undefined) {
        ...
        const contactListData = await contactListRepository.findByUserIdWithDetails(userId);
    ...
    async createChat(userIdValue: string | number | bigint | undefined, participantId: number) {
        if (!userIdValue) {
            return failure('UNAUTHORIZED', 'Session expired');
        }
        ...
        if (existingChat !== undefined) {
            return success({ chat: contactListTransformer.serialize(existingChat as any) });
        }
        ...
        return success({ chat: contactListTransformer.serialize(createdChat as any) });
    },
