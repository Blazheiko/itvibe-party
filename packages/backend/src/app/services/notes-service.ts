...
import { notesPhotoRepository, notesRepository } from '#app/repositories/index.js';
import { notesPhotoTransformer, notesTransformer } from '#app/transformers/index.js';
...
export const notesService = {
    async getNotes(
        userId: bigint,
    ): Promise<ServiceResult<{ data: ReturnType<typeof notesTransformer.serializeArray> }>> {
    ...
    async createNote(
    ...
    ): Promise<ServiceResult<{ data: ReturnType<typeof notesTransformer.serialize> }>> {
        ...
        const withPhotos = await notesRepository.findByIdAndUserId(created.id, userId);
    ...
    async updateNote(
        ...
        const withPhotos = await notesRepository.findByIdAndUserId(updated.id, userId);
        if (withPhotos === undefined) {
            return failure('NOT_FOUND', 'Note not found');
        }
    ...
    ): Promise<ServiceResult<{ photo: ReturnType<typeof notesPhotoTransformer.serialize> }>> {
        if (file === undefined) {
